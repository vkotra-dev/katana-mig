...
def test_analyze_feed_logs_field_mapping_error(monkeypatch: pytest.MonkeyPatch) -> None:
    from migrations_engine.management.fibers import _FeedAnalysisResult, _DomainObject
    class FailingAdapter:
        model_id = "test-fail"
        def call(self, system: str, user: str, response_model: type[object]) -> object:
            from migrations_engine.ai.adapter import AIResponseValidationError
            raise AIResponseValidationError(raw_response='{"bad_field": "map"}', original=Exception("schema err"))

    def mock_get_adapter(task, model_policy=None):
        if task == "feed_analysis":
            return FakeFeedAnalysisAdapter(_FeedAnalysisResult(
                lookups=[],
                domain_objects=[_DomainObject(destination_table="target_table")],
                unmatched_columns=[]
            ))
        return FailingAdapter()

    monkeypatch.setattr("migrations_engine.management.fibers.get_adapter", mock_get_adapter)
    ...
    with SessionLocal() as db:
        ...
        from migrations_engine.db.models import AICallLog
        logs = db.query(AICallLog).filter_by(project_id=project_id, call_type="feed_analysis").all()
        error_logs = [log for log in logs if log.raw_response == '{"bad_field": "map"}']
        assert len(error_logs) == 1

def test_submit_lookup_inputs_logs_error(monkeypatch: pytest.MonkeyPatch) -> None:
    class FailingAdapter:
        model_id = "test-fail"
        def call(self, system: str, user: str, response_model: type[object]) -> object:
            from migrations_engine.ai.adapter import AIResponseValidationError
            raise AIResponseValidationError(raw_response='{"bad_lookup": "map"}', original=Exception("schema err"))

    monkeypatch.setattr("migrations_engine.management.fibers.get_adapter", lambda task, model_policy=None: FailingAdapter())

    project_id, feed_id = _seed_feed_with_slice()
    with SessionLocal() as db:
        from migrations_engine.management.fibers import submit_lookup_inputs
        from migrations_engine.api.schemas import LookupInputsRequest
        from migrations_engine.ai.adapter import AIResponseValidationError
        ...
        with pytest.raises(AIResponseValidationError):
            submit_lookup_inputs(
                ...
                body=LookupInputsRequest(source_values=["a"], dest_entries=[], destination_lookup_csv="id,name\n1,one")
            )
            
        db.rollback()
            
        from migrations_engine.db.models import AICallLog
        logs = db.query(AICallLog).filter_by(project_id=project_id, call_type="lookup_mapping").all()
        assert len(logs) == 1
        assert logs[0].raw_response == '{"bad_lookup": "map"}'

def test_submit_lookup_inputs_logs_generic_error(monkeypatch: pytest.MonkeyPatch) -> None:
    class FailingAdapter:
        model_id = "test-fail"
        def call(self, system: str, user: str, response_model: type[object]) -> object:
            raise Exception("network failure")
