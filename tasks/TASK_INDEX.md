...
| [001gr-lookup-json-mutation-not-detected-bug](./001gr-lookup-json-mutation-not-detected-bug.md) | Fix a deeper, separate silent no-op: `destination_mappings` is a plain SQLAlchemy `JSON` column, and mutating an existing group dict in place before reassigning defeats SQLAlchemy's change detection, so the DB write is dropped. Affects "stack a 2nd value onto an existing destination," remove, and move. Fix via `flag_modified`. Backend-only. |
| [001gs-lookup-destination-mappings-not-reconciled-from-flat-map](./001gs-lookup-destination-mappings-not-reconciled-from-flat-map.md) | Fix a third, independent bug found while verifying [[001gr]]: `add_source_value`/`remove_source_value` silently drop sibling source values when `destination_mappings` was never explicitly seeded (only `source_value_map` was). `move_source_value` already reconciles from the flat map correctly — extract that logic into a shared helper and reuse it in all three handlers. Backend-only. |
