...
describe("navigation-access", () => {
  it("allows admin access for admin and pm roles", () => {
    expect(canAccessAdmin("admin")).toBe(true);
    expect(canAccessAdmin("pm")).toBe(true);
    expect(canAccessAdmin("central_team")).toBe(false);
  ...
  it("allows project access for member stakeholders and global roles", () => {
    expect(canAccessProject("pm", "project-1", [])).toBe(true);
    expect(canAccessProject("read_only_auditor", "project-1", [])).toBe(true);
    expect(canAccessProject("central_team", "project-1", ["project-1"])).toBe(true);
    expect(canAccessProject("central_team", "project-1", [])).toBe(false);
  ...
  it("returns role-specific visible actions", () => {
    expect(visibleActionsForRole("admin")).toContain("manage-users");
    expect(visibleActionsForRole("pm")).toContain("manage-memberships");
    expect(visibleActionsForRole("central_team")).toContain("gate-2-review");
