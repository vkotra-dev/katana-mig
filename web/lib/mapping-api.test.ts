...
describe("mapping-api", () => {
  ...
  it("loads all approved mapping snapshots for a source", async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => [
        {
    ...
    const result = await getAllApprovedMappingSnapshots("token-1", "project-1", "source-1");

    expect(fetchMock).toHaveBeenCalledWith(
      `${BASE}/projects/project-1/sources/source-1/mapping-snapshots`,
      expect.objectContaining({
        method: "GET",
        headers: expect.objectContaining({
          Authorization: "Bearer token-1",
    ...
    expect(result[0].destinationObjectName).toBe("Customer");
  });
});
