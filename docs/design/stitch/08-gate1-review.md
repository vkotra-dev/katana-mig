...
**Screen name:** `Gate 1 Review` — use this exact name as the screen/frame title.

Design the **Gate 1 Review** screen for Katana (admin approval), inside the app
shell. Use a two-pane layout: **evidence on the left, decision panel on the right.**

**Left (evidence) — three stacked panels:**
- **Domain object map** (a structured list/diagram of source-to-domain objects)
- **PII classification** (table of fields with sensitivity tags)
- **Coverage gaps** (list of unmapped or uncovered areas, amber/red)

**Right (decision panel):**
- **Approve** primary button
- **Push back** secondary, which reveals **structured target fields** (required) and a
  comment box; submission is blocked until the structured fields are filled.
- A note: "Submitting records an approval — it does not call execution directly."
...
- **Push-back state** with the structured-target-fields form expanded and a validation
  error for a missing required field.
- **Submitting / success state** ("Approval submitted").
