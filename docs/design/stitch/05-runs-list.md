...
**Screen name:** `Runs` — use this exact name as the screen/frame title.

Design the **Runs list** for Katana — a cross-project table of migration runs inside
the app shell.

**Header:** title "Runs", filter bar (project, environment, status, object), search,
and a primary **Launch run** button top-right.

**Dense table**, columns:
- monospace `run_id` (copy icon)
- Project (link)
- **Destination object**
- **Environment** tag
- Status chip (queued / running / paused / completed / failed / awaiting approval)
- Current stage
- Started (monospace timestamp)
- Last checkpoint (monospace timestamp)
- Reconciliation status (green reconciled / red failed / gray n/a)

Row hover reveals quick actions: Open; paused rows also show a role-gated Resume.
...
- **Empty state**: "No runs yet" + Launch-run CTA.
- **Loading skeleton**.
