...
**Screen name:** `Gate 2 Review` — use this exact name as the screen/frame title.

Design the **Gate 2 Review** screen for Katana (stakeholder lookup approval), inside
the app shell. The core is a **side-by-side comparison** of a Lookup Inventory
artifact and a Lookup Map artifact.

**Main: a dense table of lookup rows**, each row showing the source value, the mapped
target value, and a **state chip**: confirmed (green) / low-confidence (amber) /
unmapped (red) / overridden (blue).

**Controls:**
- a top toolbar with **Bulk approve** and a filter by state
- per-row **Override** (inline editor) and **Resolve unmapped** actions
- a primary **Submit for approval** button that is **disabled while any row is
  unmapped** (show a helper: "N unmapped values must be resolved").
...
- **Blocked-submit state**: unmapped rows present, Submit disabled, helper text.
- **Inline override editor** open on one row.
