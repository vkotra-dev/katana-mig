...
**Screen name:** `Project Detail` — use this exact name as the screen/frame title.

Design the **Project Detail** screen for Katana inside the app shell.

**Header:** project name + monospace `project_id` (copy), source type tag, current
lifecycle stage chip, environment-ordering tags, and role-gated action buttons:
**Launch run**, **Raise change request**. If blocked, a red banner with the reason.

**Tabbed body:**
1. **Overview** — a horizontal **lifecycle stepper**: ingestion → intake → planning →
   approval gate → implementation → verification → review → delivery, with the current
   stage highlighted and "days in stage" shown. Below it, a key-value metadata grid:
   goal, destination schema owner, lawful basis, contacts/escalation, definition
   version (monospace `definition_id`), assumptions, and **unresolved questions**
   highlighted in amber.
2. **Artifacts** — table of artifacts per stage (source schema, domain object map,
   mapping snapshot, lookup snapshot, codegen input, knowledge-freeze), each with a
   monospace **version pill** and produced date; row click opens an inspect drawer.
3. **Change requests** — table of active CRs with type (migration / lookup-delta) and
   status chip.
4. **Knowledge-freeze history** — versioned list with published dates.
5. **Runs** — the run-history table for this project (reuse the Runs list columns),
   showing reconciliation status per run.
...
- **Blocked state** with the red banner and reason.
- **Auditor variant**: all tabs visible, all action buttons hidden.
