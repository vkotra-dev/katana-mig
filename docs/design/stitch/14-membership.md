...
**Screen name:** `Project Membership` — use this exact name as the screen/frame title.

Design the **Project Membership** management screen for Katana (central_team only),
inside the app shell. It manages which project_stakeholder users belong to a project.

**Header:** project name + monospace `project_id`, title "Members", and an **Add
member** primary button.

**Members table**, columns: display name, email (monospace), added date, and a Remove
action per row.

**Add-member picker** (drawer/modal): a searchable list of users **limited to
project_stakeholder role**. A helper note: "central_team and read_only_auditor do not
require membership."
...
- **Add-member picker** open.
- **Duplicate-membership blocked** state (a user already a member shown disabled).
