...
**Screen name:** `Admin · Users` — use this exact name as the screen/frame title.

Design the **Admin → Users** screen for Katana (central_team only) inside the app
shell.

**Header:** title "Users", filter by role/status, search by email/name, and a primary
**Create user** button.

**User table**, columns: display name, email (monospace), **role** chip (central_team
/ project_stakeholder / read_only_auditor), **status** chip (active / disabled /
soft-deleted), created, last updated. Soft-deleted rows render grayed with a "Deleted"
chip and never disappear (audit). Row actions: Edit, Soft-delete.

**Create user drawer:** email, display name, role selector, temporary password;
duplicate-email validation inline.

**Edit user drawer:** display name + a **role change** that requires an explicit
confirm step.
...
- **Create-user drawer** open, with a duplicate-email error.
- **Soft-delete confirm dialog**: "This deactivates the account and blocks login;
