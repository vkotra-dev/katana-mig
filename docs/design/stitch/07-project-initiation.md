...
**Screen name:** `Project Initiation` — use this exact name as the screen/frame title.

Design the **Project Initiation wizard** for Katana — a focused, data-dense
multi-step form (left-rail step indicator + form panel). Steps:

**Step 1 — Identity:** project name, goal, stakeholder email, central-team assignee,
**lawful basis for processing** (required).

**Step 2 — Source:** a source-type selector (database / fixed_length_file / csv / xls
/ composite), a data-availability field, and **type-specific fields** that change
with the selection:
- database: connection ref, schema/table/view/query, filters, key hints
- fixed_length_file: file path/pattern, record length, offsets, widths, encoding,
  header/trailer rules, plus a **fixed-width spec document** upload
- csv/xls: sheet name, delimiter, headers, column hints
- composite: a repeatable list of backing sources

**Step 3 — Destination:** **destination schema DDL** input/upload, with a note that
the destination schema is client-owned and will not be invented.

**Step 4 — Environments:** an ordered, drag-to-reorder list of execution environments.

**Step 5 — Review & submit:** read-only summary of all fields; primary **Submit**
button. Submitting freezes a ProjectDefinition and opens the initial change request.
...
- **Inline validation-error state** (missing required fields highlighted).
- **Success state**: "Project frozen — definition v1 created" with a link to the new
  project.
